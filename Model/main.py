...
import webbrowser  # To open the Kibana dashboard
...
def archive_and_reset_json_file():
    ...
    if recognized_users:
        ...
        with open(new_filepath, mode='w', newline='') as file:
            ...
            # Write header
            writer.writerow(['Firstname', 'Lastname', 'Phone', 'Sex', 'Emergency Contact', 'Division', 'Status'])

            for user_data in all_users:
                user_id, firstname, lastname, phone, sex, emergency_contact, division, image_paths_str = user_data
                Status = 'Present' if any(u['firstname'] == firstname and u['lastname'] == lastname for u in recognized_users) else 'Absent'
                writer.writerow([firstname, lastname, phone, sex, emergency_contact, division, Status])
    ...
    threading.Timer(60*3, archive_and_reset_json_file).start()
...
# Open Kibana dashboard URL in the browser
kibana_dashboard_url = 'http://localhost:5601/app/dashboards#/view/a5010e68-bdac-440c-8950-ecd8512b2289?_g=(refreshInterval:(pause:!f,value:1000),time:(from:now-20s,to:now))&_a=()'
webbrowser.open(kibana_dashboard_url)  # Automatically opens the Kibana dashboard
